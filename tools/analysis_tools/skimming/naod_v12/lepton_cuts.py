...
def parent_mask(obj): return parent_mask(obj)
    
def ele_gen_mask(ele): return ele_gen_mask(ele)
def lpte_gen_mask(lpte): return lpte_gen_mask(lpte)
def muon_gen_mask(muon): return muon_gen_mask(muon)

def conv_veto(lpte): return lpte.convVeto == 1

# Define filtered masks

def signal_mask(obj): return (gflav1(obj) & parent_mask(obj))
...
def looseID(muon): return muon.looseId
def mediumID(muon): return muon.mediumId
def tightID(muon): return muon.tightId
def softID(muon): return muon.softId
def softMvaID(muon): return muon.softMvaId
